...
const cart = signal<Awaited<ReturnType<typeof invoke.wake.loaders.cart>> | null>(null)
    ...
    return (
        <div class='container mx-auto max-w-[1330px] py-6 px-4 flex flex-col gap-4 min-h-screen'>
            ...
            <div class='flex flex-col items-center'>
                <h2 class='text-xl font-bold text-black'>OBRIGADO POR COMPRAR NO ABACATE</h2>
                <span class='text-sm'>
                    O número do seu pedido é: <span class='font-bold'>{order.orderId}</span>
                </span>
                <span class='text-sm'>
                    A confirmação foi enviada para o e-mail: <span class='font-bold'>{user.value.email}</span>
                </span>
            </div>

            <a href='/' class='block mb-4 mx-auto text-sm font-bold text-white bg-black px-4 py-2'>
                CONTINUAR COMPRANDO
            </a>
            ...
            <div class='px-2 py-4 flex flex-col items-center gap-5 bg-stone-100'>
                ...
                <div class='flex flex-col items-center text-sm'>
                    ...
                    <span>Data do pedido: {new Date(order.date).toLocaleDateString('pt-BR')}</span>
                    <span>Forma de pagamento: ???</span>
                    {order.discountValue > 0 && (
                        <span>Desconto: {formatPrice(order.discountValue)?.replace(/R\$\s+/, 'R$ -')}</span>
                    )}
                ...
                {order.delivery && (
                    <div class='flex flex-col items-center text-sm gap-1'>
                        ...
                        <span>{order.delivery.cost ? formatPrice(order.delivery.cost) : 'Frete Grátis'}</span>
                        <span>
            ...
            <div class='flex flex-col px-4 gap-4'>
                {cart.value.selectedAddress && (
                    <>
                        <span class='text-lg font-medium block mt-2'>Endereço de Entrega:</span>
                        <div class='flex flex-col text-sm'>
                            ...
                            <span>
                                CEP: {cart.value.selectedAddress.cep.toString().replace(/(\d{5})(\d{3})/, '$1-$2')}
                            </span>
                        ...
                        return (
                            <div class='flex items-center py-6'>
                                ...
                                <div class='flex flex-col gap-0.5 ml-5'>
                                    <div class='font-medium text-sm uppercase'>{seller || 'ABACATE'}</div>
                                ...
                                <div class='flex flex-col gap-1 ml-auto'>{formatPrice(cartProduct.price)}</div>
                            </div>
                        )
                    })}
                </div>
