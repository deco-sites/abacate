        ...
        <>
            ...
            <button
                ...
                onClick={async () => {
                    ...
                    const input = [...(document.getElementsByName('payment') as NodeListOf<HTMLInputElement>)].find(
                        i => i.checked,
                    ) as HTMLInputElement

                    const isCard = input.getAttribute('id')?.toLowerCase().startsWith('cartão') ?? false
                    const isPix = input.getAttribute('id')?.toLowerCase().startsWith('pix') ?? false
                    const isBoleto = input.getAttribute('id')?.toLowerCase().startsWith('boleto') ?? false
            ...
            <div class='px-4 py-3 flex flex-col'>
                                ...
                                <input
                                    ...
                                    onInput={async () => {
                                        ...
                                        paymentScripts.value =
                                            cart.value?.selectedPaymentMethod?.scripts?.filter(nonNullable) ?? []
                ...
                <div class='hidden flex-col gap-6 peer-has-[input[id^=Cartão]:checked]:flex'>
                    ...
                    <form id='card-form' class='flex flex-col gap-3'>
                        ...
                        <div class='flex flex-col gap-1 w-full'>
                            <span class='font-medium text-sm'>Nome do titular (Como escrito no cartão)</span>
                            <input
                                type='text'
                                required
                                class='p-2 border border-stone-400 h-11'
                                ...
                                <select
                                    ...
                                    onChange={async e => {
                                        ...
                                        await invoke.wake.actions.selectInstallment({
                                            ...
                                            selectedPaymentMethodId: cart.value?.selectedPaymentMethod?.id as string,
                                        })
                                    }}
                                >
                                    {paymentMethodsPrices.value?.installmentPlans
                                        ...
                                        .map(i => (
                                            <option
                                                value={i.number}
                                            >{`${i.number === 1 ? 'À vista' : `${i.number} parcelas`} ${formatPrice(i.value)} ${i.fees ? 'com' : 'sem'} juros`}</option>
                                        ))}
                                </select>
                            </div>
                        </div>
